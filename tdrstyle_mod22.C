...
TString extraText   = "Work in Prorgress";
...
CMS_lumi( TPad* pad, int iPeriod, int iPosX )
{
  ...
  TString lumi_16 = "35.9 fb^{-1}";
  TString lumi_17 = "41.5 fb^{-1}";
  TString lumi_18 = "59.8 fb^{-1}";
  TString lumiText;
  if (iPeriod == 1) {
    lumiText += lumi_16;
    lumiText += " (13 TeV)";
  ...
  if( !outOfFrame ) {
    if( drawLogo ) {
      ...
      cout << cmsTextFont << " " << cmsTextSize << " " << align_ << " " << iPeriod << " " << iPosX << " No logo " << posX_ << " " << posY_ << " " << cmsText << " " << extraText << endl;
      if( writeExtraText ) {
...
TCanvas* tdrDiCanvas(const char* canvName, TH1 *hup, TH1 *hdw,
  ...
  // references for T, B, L, R
  float T_ref = (square ? 0.07 : 0.08);
  float B_ref = (square ? 0.13 : 0.12);
  float L = (square ? 0.15 : 0.12);
  float R = (square ? 0.05 : 0.04);
  ...
  hup->GetYaxis()->SetTitleOffset((square ? 1.25 : 1) * Hup / H_ref);
  hup->GetXaxis()->SetTitleOffset(square ? 1.0 : 0.9);
  ...
  assert(hdw);
  // Scale text sizes and margins to match normal size
  hdw->GetYaxis()->SetTitleOffset((square ? 1.25 : 1) * Hdw / H_ref);
  hdw->GetXaxis()->SetTitleOffset(square ? 1.0 : 0.9);
